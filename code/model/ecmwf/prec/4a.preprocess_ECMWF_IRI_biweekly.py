...
week = "full" 2-week, days = 14 days in 2-week, members = ensemble members). The repacked
...
ec_daily = ec_daily - 99999 #for checking missing values
...
#---------------------------------------------------------------
# 4. Generate the ECMWF daily rainfall percentile values
#---------------------------------------------------------------
print('Finding ECMWF daily rainfall '+ str(threshold) + 'th percentile value...')

ec_climatology_mask = np.empty([lead_times,len(week_initial_date),len(ec_lat),len(ec_lon)])
...
ec_filename = 'ECMWF_' + calendar.month_abbr[target_month] + '_Data_BiWeekly.nc'
...
if plot_figure:
   ...
   for i_step in range(0,lead_times):
       ...
       end_date   = "%02d"%target_month + "%02d"%(int(start_date)+13)
