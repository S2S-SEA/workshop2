'''
1. Load the 'daily rainfall XXth percentile climatology mask' and 'ECMWF_Data_BiWeekly'
netCDF files.
2. Calculate 'NDD' in 2-week from the 'ECMWF_Data_BiWeekly' dataset,
by applying the 'daily Rainfall XXth percentile climatology mask' and summing up the
total number of dry days in 2-week.
3. Output/Plot the 'NDD' in 2-week for ECMWF model climatology/total/anomaly.
'''
...
#--------------------------------------------------------------------------------------------------
#Read 'ECMWF_Data_BiWeekly' data
cur_ec_filename = 'ECMWF_' + calendar.month_abbr[target_month] + '_Data_BiWeekly.nc'
...
#---------------------------------------------------------------------------------------------
# 2. This part is to calculate ECMWF (NDD) in 2-week [climatology/total/anomaly]
#---------------------------------------------------------------------------------------------
...
ec_total_ens     = np.sum(ec_data,axis=3) #Sum up all 14 days in 2 weeks and compress the 'days' axis. Use np.sum along axis3
...
if data_output == True:
   ...
   ec_filename = 'ECMWF_' + calendar.month_abbr[target_month] + '_threshold' + str(threshold) + '_Climatology_BiWeekly.nc'
   ...
   ec_filename = 'ECMWF_' + calendar.month_abbr[target_month] + '_threshold' + str(threshold) + '_Total_BiWeekly.nc'
...
if plot_figure == True:
   ...
   for i_step in range(0,lead_times):
       ...
       end_date   = "%02d"%(int(start_date[-2:])+13)

       data_range = [0,days]
       title_str  = 'ECMWF NDD BiWeekly Climatology' + '\n' + start_date + '-' + end_date + ' (LT' + str(i_step+1) + ')'
       name_str   = plot_dir + 'ECMWF_' + start_date + '-' + end_date + '_' + 'LT' + str(i_step+1) + '_threshold' + str(threshold) + '_Climatology_BiWeekly.png'
       ...
       data_range = [0,days]
       title_str  = 'ECMWF NDD BiWeekly Total' + '\n' + str(target_year) + ' ' + start_date + '-' + end_date + ' (LT' + str(i_step+1) + ')'
       name_str   = plot_dir + 'ECMWF_' + str(target_year) + '_' + start_date + '-' + end_date + '_' + 'LT' + str(i_step+1) + '_threshold' + str(threshold) + '_Total_BiWeekly.png'
       ...
       title_str  = 'ECMWF NDD BiWeekly Anomaly' + '\n' + str(target_year) + ' ' + start_date + '-' + end_date + ' (LT' + str(i_step+1) + ')'
       name_str   = plot_dir + 'ECMWF_' + str(target_year) + '_' + start_date + '-' + end_date + '_' + 'LT' + str(i_step+1) + '_threshold' + str(threshold) + '_Anomaly_BiWeekly.png'
