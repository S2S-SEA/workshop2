...
#---------------------------------------------------------------------
# 3. Generating TRMM biweekly rainfall 'XXth percentile climatology mask'
#---------------------------------------------------------------------
print ('Preparing TRMM biweekly rainfall ' + str(threshold) + 'th percentile climatology mask...')
...
print ('Mean of TRMM biweekly rainfall ' + str(threshold) + 'th percentile climatology mask: ' + str(np.mean(trmm_climatology_mask)))
print ('Median of TRMM biweekly rainfall ' + str(threshold) + 'th percentile climatology mask: ' + str(np.median(trmm_climatology_mask)))
print ('Done!')

#--------------------------------------------------------------------------------
# 4. This part is to calculate TRMM NDD/NWD in a 2-week (climatology/total/anomaly)
#--------------------------------------------------------------------------------
if method == 'NDD':
    #Convert based on the TRMM 'Rainfall XXth percentile climatology mask' with minimum value of XXmm/day (XX=dynamic?)
    trmm_climatology_mask[trmm_climatology_mask < 1] = 1
elif method == 'NWD':
    #Convert based on the TRMM 'Rainfall XXth percentile climatology mask' with maximum value of XXmm/day (XX=dynamic?)
    trmm_climatology_mask[trmm_climatology_mask < 1] = 1
#else:
...
if data_output == True:
   ...
   trmm_filename = 'TRMM_' + calendar.month_abbr[target_month] + '_threshold' + str(threshold) + '_Climatology_BiWeekly_' + method + '.nc'
   s2s_utility_prec.write_trmm(trmm_output,trmm_filename,trmm_climatology,trmm_week,trmm_year,ec_lat,ec_lon,'Climatology')

   trmm_filename = 'TRMM_' + calendar.month_abbr[target_month] + '_threshold' + str(threshold) + '_Total_BiWeekly_' + method + '.nc'
   s2s_utility_prec.write_trmm(trmm_output,trmm_filename,trmm_total,trmm_week,trmm_year,ec_lat,ec_lon,'Total')

   trmm_filename = 'TRMM_' + calendar.month_abbr[target_month] + '_threshold' + str(threshold) + '_Anomaly_BiWeekly_' + method + '.nc'
...
if plot_figure == True:
   ...
   end_date = start_date[:2] + "%02d"%(int(start_date[-2:])+13)

   data_range = [0,14]    #change data range for plotting accordingly
   title_str = 'TRMM ' + method + ' Biweekly Climatology' + '\n' + start_date + '-' + end_date
   name_str = plot_dir + 'TRMM_' + start_date + '-' + end_date + '_threshold' + str(threshold) + '_Climatology_BiWeekly_' + method + '.png'
   ...
   data_range = [0,14]
   title_str = 'TRMM ' + method + ' Biweekly Total' + '\n' + str(target_year) + ' ' + start_date + '-' + end_date
   name_str = plot_dir + 'TRMM_' + str(target_year) + '_' + start_date + '-' + end_date + '_threshold' + str(threshold) + '_Total_BiWeekly_' + method + '.png'
   ...
   data_range = [-14,14]
   title_str = 'TRMM ' + method + ' Biweekly Anomaly' + '\n' + str(target_year) + ' ' + start_date + '-' + end_date
   name_str = plot_dir + 'TRMM_' + str(target_year) + '_' + start_date + '-' + end_date + '_threshold' + str(threshold) + '_Anomaly_BiWeekly_' + method + '.png'
